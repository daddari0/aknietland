...
export function Gallery({ language }: GalleryProps) {
  ...
  return (
    <section id="gallery" className="py-20 lg:py-28 bg-white relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-80 h-80 rounded-full bg-[#FAC315]/8 blur-3xl" />
      <div className="absolute bottom-0 left-0 w-80 h-80 rounded-full bg-[#1CA6D0]/8 blur-3xl" />

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 bg-[#FAC315]/10 border border-[#FAC315]/25 text-[#b88a00] text-sm font-bold px-5 py-2 rounded-full mb-5">
            {t.badge}
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {photoData.map((photo, index) => (
            <div
              key={index}
              className={`relative group cursor-pointer overflow-hidden rounded-[28px] shadow-lg hover:shadow-2xl transition-all duration-400 hover:-translate-y-2 ${
            ...
            >
              ...
              {/* Tag badge */}
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm text-gray-800 text-xs font-black px-3 py-1.5 rounded-full shadow-md translate-y-2 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                {photo.tag}
              </div>
